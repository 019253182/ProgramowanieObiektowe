...
void menu(Tablica* arr){
	...
	while(nr!=13){
		...
		int lkolumn, lwierszy, ile, ktora;
		ktora=0;
		cout << "Liczba kolumn: ";
		cin >> lkolumn;
		for(int i=1;i<=lkolumn;i++)
		{
			arr->tab[i]=1;
		}
		cout<<"Liczba wierszy: ";
		cin >> lwierszy;
		cout<<"Ile kolumn ma byc typu string:";
		cin>>ile;
		cout<<"Ktore kolumny maja byc string:";
		for(int i=0;i<ile;i++)
		{
			cin>>ktora;
			arr->tab[ktora]=2;
		...
		if ((wybierz_k>arr->lkolumn)||(wybierz_k<1)||(wybierz_w>arr->lwierszy)||(wybierz_k<1))
		{
	        ...
	        {
                	arr->zmianaZawart( wybierz_w, wybierz_k /**arr->tab[wybierz_k]*/);
		}
		break;		
